...
async function loadContent() {
    ...
    list.innerHTML = data.results.map((item, i) => `<li class="member">
        <img  class="avatar" src="${item.picture.thumbnail}" alt="avatar image">
        <span class="name">${item.name.first} ${item.name.last}</span>
        <div class="tooltip">
            <h2>Add to the event?</h2>
            <div class="buttons">
                <input type="radio" id="radio-${2 * i}" name="radio--${i}" value="yes">
                <label for="radio-${2 * i}">
                    <button class="btn" id="yes">yes</button>
                </label>
                <input type="radio" id="radio-${2 * i + 1}" name="radio--${i}" value="no">                <label for="radio-${2 * i + 1}">
